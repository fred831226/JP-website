...
Services & projects begins with a deep-navy service Hero and six approved capability labels, then shifts to the light content canvas for an evidence-led vertical project list. Every project section pairs a large rights-cleared media region with approved type/title, concise context, optional scope/outcome, and related-product context. Evidence is read in place and the section is not a whole-block link; a separately labelled related-Series or Contact link may appear only when approved. AI atmosphere and generic stock media never enter this surface.
...
- **Project evidence block** — Rights-cleared project media and approved facts receive equal hierarchy. Use one semantic in-place evidence section per approved Project on Services & projects, with a large media region, title/type, concise scope/outcome, and related-product context only when approved. The section is not a whole-block link in V1; optional related-Series or Contact links are separate, labelled actions. Never use AI atmosphere or generic stock media as evidence.
