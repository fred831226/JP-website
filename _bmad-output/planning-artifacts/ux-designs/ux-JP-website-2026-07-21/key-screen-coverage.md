...
| Services & projects | [mockup](mockups/key-screen-services-projects-responsive-01.html) | Owns the service-capability Hero and complete in-place verified Project evidence; prevents evidence from being styled like generic marketing cards or implying individual detail routes |
...
V1 has no sign-in, account recovery, CMS workspace, News/Project editor, media picker, custom product-maintenance UI, backend, or database. Project evidence and all other V1 content use the external Git/validation/Vercel workflow defined in EXPERIENCE.md: source preparation, change review, validation report, Vercel Preview, JP PUMP approval, Production promotion, and named deployment rollback.
...
- Latest Information/News is absent from all V1 IA, navigation, publishing and coverage.
- Individual Project detail pages are deferred; Services & projects is the complete V1 Project-evidence surface, and Home teasers target that page or an in-page anchor.
- Brand, Pump Type and Purpose pages remain spine-only V1 surfaces under `/zh-tw/`.
- All five approved mockups are retained without redraw; implementation only corrects any hypothetical Project-detail link semantics.
- Optional UX validation/reviewer gate is skipped by user decision.
- Approved company facts, contact data, partners, product data, and Project evidence remain content dependencies, not UX blockers.
